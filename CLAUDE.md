...
このリポジトリは **単一コードベース** から複数の派生版 (variant) を別々の拡張機能として公開できる「バリアント方式」を採用している。 現在 `default` (フォント選択 UI 付き + シンプルモードトグル) と `notosans` (Noto Sans JP + UDEV Gothic JPDOC 固定版) の 2 variant が定義されており、 それぞれ独立した拡張機能 ID として Chrome Web Store / Firefox AMO に並行リリースされている。

v3.0.4 で default にも **シンプルモードトグル** (`SIMPLE_MODE_LOCKED` in `font-config.js`) を追加した。 ユーザーの使い分け:
- **default + simpleMode OFF**: 普段はフォント選択 UI で好きなフォントを選ぶ
- **default + simpleMode ON**: トグルだけで Noto Sans JP + UDEV Gothic JPDOC に固定
- **notosans variant**: 最初から固定、 フォント選択 UI なし (popup がシンプル)
...
|---|---|---|---|
| `default` | ユーザー選択 (6×3×2=36プリセット) + simpleMode トグル | `...323f` | 通常版 |
