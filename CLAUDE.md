...
公開済み variant の退役 (variant ディレクトリの削除 / publish.yml matrix からの除外 / `.json` の削除) は **技術的判断ではなくビジネス判断** として扱う。 「機能を default に統合できる」「YAGNI / scope creep / 冗長」といった技術的合理性を根拠に自動退役してはいけない。 退役で毀損するものは:
...
**過去事例 (2026-05-15)**: `/rere` Agent D の YAGNI 指摘 (「notosans variant は default に新機能 (simpleMode トグル) を追加すれば吸収できる」) を機械的に「全部修正」したことで、 (1) default に simpleMode という新機能を無断追加、 (2) notosans variant を退役、 という 2 段階の暴走が起きた。 直後に「なんで notosans やめた?」「シンプルモードって無断で追加したよね?」とユーザーから気付かれて、 simpleMode 機能本体 + notosans 退役を全面撤回した。 教訓: 「機能の重複」を理由にした退役推奨は判定カテゴリ `business-call` であり、 さらに「重複を解消するために新機能を増設する」 は二重に business-call (新機能追加もユーザー承認マター)。
...
⚠️ **過去の罠 (v3.0.3 時点で潜伏)**: 中間実装で `const MARKER = '\x01'` (ASCII 制御文字 SOH、 目視で空文字に見える) を split/join 区切り文字として使っていたことがあった。 「動いてはいるが、 エディタの自動正規化 / git の `core.autocrlf` / 一部 linter で剥がれると `MARKER = ''` に化けて、 次の `npm run generate-css` で 36 preset 全壊する」時限爆弾。 v3.0.4 で escape → 直接 `${B}` 置換 + sanity check (`literalBody.includes('${B}')` 必須) に書き換えて根絶。 **今後 preset 生成で `''` に見える区切り文字を使ったコードを見つけたら、 `PowerShell` の `[regex]::Match($content, "const MARKER = '(.*?)';").Groups[1].Value` を bytes hex dump して中身を確認すること**。
...
### Service Worker のライフサイクル (chrome.alarms 移行)

v3.0.4 で `setTimeout` ベースの debounce / retry を **`chrome.alarms`** に移行した。 理由:

- **Chrome MV3 の SW は 30 秒 idle で evict される**。 evict 時に **`setTimeout` がキャンセルされる** (timer 自体が CallbackQueue ごと消える)
- `chrome.alarms` は OS レベルで永続化されているため、 SW が evict されても次回 wake-up 時に確実に発火する
- `alarms` permission を manifest に明示的に追加する必要あり (`permissions: ["storage", "scripting", "alarms"]`)

具体的な配置:
- **debounce**: `chrome.storage.onChanged` listener が `chrome.alarms.create(ALARM_DEBOUNCE, { delayInMinutes: 0.0025 })` を呼ぶ。 約 0.15 秒後に発火し、 settings の最新値を fetch して `ensureRegistration` を呼ぶ
- **retry**: `chrome.scripting.registerContentScripts` 失敗時、 exponential backoff (1m → 5m → 30m → 60m cap) で `chrome.alarms.create(ALARM_RETRY, ...)` を schedule。 構造化エラーログを `_last_register_error` キーで chrome.storage.local に記録

これにより「設定変更したのに反映されない」(SW evict 中に setTimeout 消失) silent bug が解消された。
