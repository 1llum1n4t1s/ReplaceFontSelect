...
### Architecture
- **default variant に simpleMode トグル追加**: popup の「シンプルモード」 ON で `SIMPLE_MODE_LOCKED` (Noto Sans JP + UDEV Gothic JPDOC + Regular) を強制適用。 notosans variant 相当の機能を default にも提供
- **notosans variant は引き続き運用**: 独立ブランドとして `{ee49eeb2-93a9-4062-ba75-06610054323e}` で Chrome Web Store / Firefox AMO 公開を継続。 「最初からシンプル」を求めるユーザー向けに残す。 v3.0.4 では default と同じ exclude_matches 17 ドメイン + Bootstrap 5 対応 + 全 hot path 最適化 を反映
