...
[![Version](https://img.shields.io/badge/version-1.0.26-blue.svg)](https://github.com/1llum1n4t1s/ReplaceFontSelect)
...
日本語・英語どちらのサイトでも動作し、置換は **CJK（漢字・かな・記号）文字のみ**にスコープされているため、Latin テキストは元のデザインのまま保たれます。
...
- 🌐 **多言語対応**: `unicode-range` により CJK 文字のみを置換するため、英語サイト・バイリンガルサイトでも Latin 部分は元のデザインのまま自然に表示されます。
...
Font replacement is scoped to **CJK characters only** via `unicode-range`, so Latin text on English or bilingual sites keeps its original appearance.
...
- 🌐 **Multilingual-friendly**: `unicode-range` limits replacement to CJK characters, leaving Latin text untouched on English / bilingual sites.
