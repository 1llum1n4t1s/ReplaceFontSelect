...
`;
  ...
  // (P1-2) ASCII case-insensitive で family 重複を排除する。
  // CSS の font-family マッチング仕様は ASCII 大文字小文字を区別しないため、
  // 'MS Gothic' と 'ms gothic' は同一エントリと見なせる。 片方だけ残して @font-face 数を圧縮。
  const dedupeFamiliesAsciiCI = (families) => {
    const seen = new Set();
    const result = [];
  ...
  const sections = outputConfig.configs.map(item =>
    dedupeFamiliesAsciiCI(item.families).map(family => generateFontFace(family, item.config)).join('\n')
  );
